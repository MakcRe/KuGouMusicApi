...
`platform`: 平台类型；传 `lite` 时使用概念版签名参数（`appid=3116` 时会自动识别为概念版）
...
**调用例子：** `/user/cloud/match?hash=e1fe087ba28766b8c95239487a0e46dc&platform=lite`
...
`platform`: 平台类型；传 `lite` 时使用概念版 RSA 公钥和签名参数（`appid=3116` 时会自动识别为概念版）
