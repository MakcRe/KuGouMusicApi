...
说明：登录后调用此接口，可以获取用户的听歌偏好设置（性别、年龄段、语言、风格等）

**可选参数：**

`token`: 登录令牌（也可通过 cookie、request body 传递）

**接口地址：** `/user/preference`

**调用例子：** `/user/preference`
...
其中 `lang`/`style` 为基础偏好 JSON 字符串；`song_lang` 为推荐强度 JSON 字符串，格式为 `{标签id:权重}`，**50 为默认强度，低于 50 减少推荐，高于 50 加大推荐，0 屏蔽**。两组字段的标签 ID 独立，不应互相转换；`gender`/`age` 为单个 id 字符串。

### 更新听歌偏好设置

说明：登录后调用此接口，可以更新用户的听歌偏好设置。更新字段：`gender`/`age` 直接传 id，`lang`/`style`/`song_lang` 传 JSON 字符串（URL 传递时注意将 `{`、`"` 编码，或改放 request body）。传空字符串可清除对应项。

**可选参数：**（至少提供一项）

`mode`: 推荐模式，`0` 默认、`1` 熟悉、`2` 尝鲜
...
`song_lang`: 推荐强度 JSON 字符串，权重为 0–100（0 屏蔽、50 默认、100 加大）。标签：`L1` 国语、`L2` 英语、`L5` 粤语、`L4` 韩语、`L3` 日语、`L6` 纯音乐、`S9` DJ、`T1` 翻唱、`N2` 现场、`S28` AI 歌曲。

推荐模式及强度标签依据[酷狗官方偏好页面](https://h5.kugou.com/settings/v-3e4978eb/index.html)。语种不可全部屏蔽；加大 DJ 强度时不可同时屏蔽翻唱。

`stylerec_taglist`: 推荐风格标签

`wish_taglist`: 想要听的标签

`inactive_taglist`: 不想听的标签

`inactive_singerlist`: 不想听的歌手

**接口地址：** `/user/preference/update`

**调用例子：** （设置：男 + 90后 + 国语 + 流行）

```
/user/preference/update?gender=2&age=2&lang={"1":100}&style={"1":100}
```

URL 编码写法：`/user/preference/update?gender=2&age=2&lang=%7B%221%22%3A100%7D&style=%7B%221%22%3A100%7D`

清除某字段：`/user/preference/update?lang=` （空字符串）
...
> 注：以上为标准版安卓的标签 id，不同平台/市场可能存在差异。
...
说明 : 调用此接口可以调整歌单内歌曲的先后顺序（"自定义排序"，需要登录），可对用户自建歌单、收藏歌单以及"我喜欢"歌单使用。排序结果即歌单歌曲接口返回的 `sort` 字段，成功后同一歌单在新版本客户端内按新顺序展示。

如需还原/调整"我喜欢"歌单，请先通过 [`/user/playlist`](#获取用户歌单) 找到名为 `我喜欢`（`type=0`）的歌单拿到其 `listid`。

**必选参数：**

listid: 用户歌单 listid

list_ver: 歌单列表版本号，可从 [`/playlist/track/all/new`](#获取歌单所有歌曲新版) 的响应 `data.list_ver` 获取

data: 排序数据，格式为 `fileid|sort`，每项之间用逗号分隔。`fileid` 为歌曲在歌单中的 fileid，`sort` 为该歌曲的新排序权重（从 0 开始依次递增）；仅需传入被移动的歌曲即可

**可选参数：**

`type`: 歌单类型，0：自建/我喜欢，1：收藏歌单，默认为 0
...
说明 : 调用此接口可以调整"我的歌单"页中歌单的展示顺序（需要登录），自建歌单与收藏歌单分别按各自 `type` 独立排序。

**必选参数：**

total_ver: 歌单总版本号，可从 [`/user/playlist`](#获取用户歌单) 的响应 `data.total_ver` 获取

data: 排序数据，格式为 `listid|type|sort`，每项之间用逗号分隔。`listid` 为歌单 id，`type` 为歌单类别（0：自建歌单，1：收藏歌单），`sort` 为该歌单在该类别下的新排序序号（从 0 开始依次递增）；建议全量传入当前歌单数据
...
`id`: 歌单中的 `global_collection_id`（"我喜欢"等用户歌单请使用列表中的 `global_collection_id`，或使用下面的新版接口按 `listid` 查询）
...
`needsort `: 是否返回歌曲在歌单中的自定义排序序号 `sort`（传 1 返回，0 不返回），默认为 1
...
说明 : 调用此接口，传入对应的歌单 listid，即可获得对应的所有歌曲, 目前该接口仅支持 用户所创建及收藏的歌单（含"我喜欢"歌单）
...
`needsort `: 是否返回歌曲在歌单中的自定义排序序号 `sort`，默认为 1

> 说明：`data.info[]` 中每首歌的 `sort` 字段即为该歌曲在歌单中的排序权重（"自定义排序"），`collecttime` 为歌曲加入时间，按两种字段排序即可实现"从新到旧/从旧到新"等排序视图；修改排序请调用 [`/playlist/tracks/sort`](#对歌单内歌曲排序)。
...
说明：获取歌曲播放器使用的旧弹幕池。该弹幕池的资源代码为 `articulossong`，与 `/comment/music` 使用的普通歌曲评论池不同，不应把两者数量混为一谈。不需要登录。
