...
23. [`专辑详情`](#专辑详情)
24. [`专辑音乐列表`](#专辑音乐列表)
25. [`获取音乐 Auth`](#获取音乐-auth)
26. [`获取音乐 URL`](#获取音乐)
27. [`获取音乐 URL（新版）`](#获取音乐-url新版)
28. [`获取音乐 URL （Auth 版本）`](#获取音乐-url-auth-版本)
29. [`获取音乐 URL （Auth 聚合版本）`](#获取音乐-url-auth-聚合版本)
30. [`获取歌曲高潮部分`](#获取歌曲高潮部分)
31. [`搜索`](#搜索)
32. [`默认搜索关键词`](#默认搜索关键词)
33. [`综合搜索`](#综合搜索)
34. [`热搜列表`](#热搜列表)
...
41. [`音效歌单`](#音效歌单)
42. [`获取歌单详情`](#获取歌单详情)
43. [`获取歌单所有歌曲`](#获取歌单所有歌曲)
44. [`获取歌单所有歌曲（新版）`](#获取歌单所有歌曲新版)
45. [`相似歌单`](#相似歌单)
46. [`获取主题歌单所有歌曲`](#获取主题歌单所有歌曲)
47. [`获取主题音乐`](#获取主题音乐)
48. [`获取主题音乐详情`](#获取主题音乐详情)
49. [`歌曲推荐`](#歌曲推荐)
50. [`获取歌手和专辑图片`](#获取歌手和专辑图片)
51. [`获取歌手图片`](#获取歌手图片)
52. [`获取音乐相关信息`](#获取音乐相关信息)
53. [`获取更多音乐版本`](#获取更多音乐版本)
54. [`获取音乐伴奏信息`](#获取音乐伴奏信息)
55. [`获取音乐k歌数量`](#获取音乐-k-歌数量)
56. [`获取音乐详情`](#获取音乐详情)
57. [`获取音乐专辑/歌手信息`](#获取音乐专辑/歌手信息)
58. [`私人 FM(对应手机和 pc 端的猜你喜欢)`](#私人-fm对应手机和-pc-端的猜你喜欢)
59. [`banner`](#banner)
60. [`乐库 banner`](#乐库-banner)
61. [`乐库电台`](#乐库电台)
62. [`乐库`](#乐库)
63. [`电台 - 推荐`](#电台-推荐)
64. [`电台`](#电台)
...
69. [`编辑精选歌单`](#编辑精选歌单)
70. [`编辑精选专区`](#编辑精选专区)
71. [`编辑精选专区详情`](#编辑精选专区详情)
72. [`领取 VIP（需要登陆，该接口为测试接口，仅限概念版使用）`](#领取-vip需要登陆该接口为测试接口仅限概念版使用)
73. [`领取一天 VIP（需要登陆，该接口为测试接口，仅限概念版使用）`](#领取一天-vip需要登陆该接口为测试接口仅限概念版使用)
74. [`获取当月已领取 VIP 天数（需要登陆，该接口为测试接口，仅限概念版使用）`](#获取当月已领取-vip-天数需要登陆该接口为测试接口仅限概念版使用)
75. [`获取已领取 VIP 状态（需要登陆，该接口为测试接口，仅限概念版使用）`](#获取已领取-vip-状态需要登陆该接口为测试接口仅限概念版使用)
76. [`获取歌手列表`](#获取歌手列表)
77. [`获取歌手详情`](#获取歌手详情)
78. [`获取歌手专辑`](#获取歌手专辑)
79. [`获取歌手单曲`](#获取歌手单曲)
...
81. [`关注歌手`](#关注歌手)
82. [`取消关注歌手`](#取消关注歌手)
83. [`获取关注歌手新歌`](#获取关注歌手新歌)
84. [`获取视频 url`](#获取视频-ur)
85. [`获取歌曲 MV`](#获取歌曲-MV)
86. [`获取视频相关信息`](#获取视频相关信息)
87. [`获取视频详情`](#获取视频详情)
88. [`新歌速递`](#新歌速递)
89. [`场景音乐列表`](#场景音乐列表)
90. [`场景音乐详情`](#场景音乐详情)
91. [`获取场景音乐讨论区`](#获取场景音乐讨论区)
92. [`获取场景音乐模块 Tag`](#获取场景音乐模块)
93. [`获取场景音乐歌单列表`](#获取场景音乐歌单列表)
94. [`获取场景音乐视频列表`](#获取场景音乐视频列表)
95. [`获取场景音乐音乐列表`](#获取场景音乐音乐列表)
96. [`每日推荐`](#每日推荐)
97. [`历史推荐`](#历史推荐)
98. [`风格推荐`](#风格推荐)
99. [`排行列表`](#排行列表)
...
105. [`歌曲评论数`](#歌曲评论数)
106. [`歌曲评论`](#歌曲评论)
107. [`歌曲评论-根据分类返回`](#歌曲评论-根据分类返回)
108. [`歌曲评论-根据热词返回`](#歌曲评论-根据热词返回)
109. [`楼层评论`](#楼层评论)
110. [`歌单评论`](#歌单评论)
111. [`专辑评论`](#专辑评论)
112. [`提交听歌历史`](#提交听歌历史)
...
114. [`刷刷`](#刷刷)
115. [`AI 推荐`](#ai-推荐)
116. [`曲谱 - 获取歌曲曲谱`](#曲谱---获取歌曲曲谱)
117. [`曲谱 - 曲谱排行榜`](#曲谱---曲谱排行榜)
118. [`曲谱 - 曲谱详情`](#曲谱---曲谱详情)
119. [`曲谱 - 曲谱广场`](#曲谱---曲谱广场)
120. [`曲谱 - 曲谱标签`](#曲谱---曲谱标签)
121. [`频道 - 获取用户所有频道`](#频道-获取用户所有频道)
122. [`频道 - 详情`](#频道---详情)
123. [`频道 - 频道安利`](#频道-频道安利)
124. [`频道 - 相似频道`](#频道-相似频道)
125. [`频道 - 订阅`](#频道-订阅)
126. [`频道 - 音乐故事`](#频道-音乐故事)
127. [`频道 - 音乐故事详情`](#频道-音乐故事详情)
128. [`动态 - 最常访问`](#动态-最常访问)
129. [`获取用户公开的音乐`](#获取用户公开的音乐)
130. [`听书 - 每日推荐`](#听书-每日推荐)
131. [`听书 - 排行榜推荐`](#听书-排行榜推荐)
132. [`听书 - VIP 推荐`](#听书-vip-推荐)
133. [`听书 - 每周推荐`](#听书-每周推荐)
134. [`听书 - 专辑详情`](#听书-专辑详情)
135. [`听书 - 专辑音乐列表`](#听书-专辑音乐列表)
136. [`歌曲详情 - 歌曲成绩单`](#歌曲详情-歌曲成绩单)
137. [`歌曲详情 - 歌曲成绩单详情`](#歌曲详情---歌曲成绩单详情)
138. [`听歌识曲`](#听歌识曲)
139. [`获取已购单曲`](#获取已购单曲)
140. [`获取已购专辑`](#获取已购专辑)
141. [`上传音乐到云盘`](#上传音乐到云盘)
...
144. [`获取内容黑名单`](#获取内容黑名单)
145. [`导入外部歌单`](#导入外部歌单)
146. [`获取歌手单曲（新版）`](#获取歌手单曲新版)
147. [`组队领取VIP - 获取本期活动信息（需要登陆，该接口为测试接口，仅限概念版使用）`](#组队领取-vip---获取本期活动信息需要登陆该接口为测试接口仅限概念版使用)
148. [`组队领取VIP - 获取用户组队信息（需要登陆，该接口为测试接口，仅限概念版使用）`](#组队领取-vip---获取用户组队信息需要登陆该接口为测试接口仅限概念版使用)
149. [`组队领取VIP - 获取用户自己的队伍信息（需要登陆，该接口为测试接口，仅限概念版使用）`](#组队领取-vip---获取用户自己的队伍信息需要登陆该接口为测试接口仅限概念版使用)
150. [`组队领取VIP - 创建用户自己的组队队伍（需要登陆，该接口为测试接口，仅限概念版使用）`](#组队领取-vip---创建用户自己的组队队伍需要登陆该接口为测试接口仅限概念版使用)
151. [`组队领取VIP - 加入组队队伍（需要登陆，该接口为测试接口，仅限概念版使用）`](#组队领取-vip---加入组队队伍需要登陆该接口为测试接口仅限概念版使用)
152. [`音效 - 耳机列表`](#音效---耳机列表)
153. [`音效 - 耳机音效`](#音效---耳机音效)
154. [`音效 - 通用耳机音效`](#音效---通用耳机音效)
155. [`音效 - 明星音效`](#音效---明星音效)
156. [`音效 - 汽车列表`](#音效---汽车列表)
157. [`音效 - 汽车音效`](#音效---汽车音效)
