...
const resolvePlatform = (params) => {
  const requestAppid = params?.appid || appid;
  const isLiteRequest =
    params?.platform === 'lite' ||
    params?.cookie?.KUGOU_API_PLATFORM === 'lite' ||
    Number(requestAppid) === Number(liteAppid);

  return {
    appid: requestAppid,
    clientver: params?.clientver || (isLiteRequest ? liteClientver : clientver),
  };
};
...
module.exports = (params, useAxios) => {
  ...
  return new Promise(async (resolve) => {
    try {
      const { appid: requestAppid, clientver: requestClientver } = resolvePlatform(params);
